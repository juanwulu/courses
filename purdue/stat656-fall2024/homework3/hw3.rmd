...
hierarchical_model_code <- "
    data {
        ...
        real<lower=0> pr_std;           // Prior standard deviation of means
        vector[n * t] color;            // Color of the word in each trial
        vector[n * t] word;             // Word in each trial
        array[n * t] int<lower=0> y;    // Reaction time
    }

    parameters {
        vector<lower=0>[n] std;    // Emission standard deviation
        vector[n] mu_congruent;    // Mean reaction time for congruent condition
        vector[n] mu_incongruent;  // Mean reaction time for incongruent condition
    }

    transformed parameters {
        vector[n*t] congruent;   // Binary indicator for congruency
        for (i in 1:n*t) {
            congruent[i] = (color[i] == word[i]);
        }
        vector[n*t] mu;
        for (i in 1:n*t) {
            mu[i] = mu_congruent[i] * congruent[i] + mu_incongruent[i] * (1 - congruent[i]);
        }
    }

    model {
        mu_congruent ~ lognormal(0, pr_std);
        mu_incongruent ~ lognormal(0, pr_std);
        std ~ gamma(1, 1);
        y ~ lognormal(mu, std);
    }

    generated quantities {
        array[n*t] real y_hat;
        y_hat = lognormal_rng(mu, std);
    }
"

...
The log conditional posterior distribution of $\theta{j}$ is implemented as follows:

```{r log-posterior-theta}
log_posterior_theta <- function(theta, mu, tau_sq, sigma_sq, n, y_bar) {
  precision_theta <- 1 / (1 / tau_sq + n / sigma_sq)
  mu_hat_theta <- (mu / tau_sq + n * y_bar / sigma_sq) / precision_theta
  log_prob <- -0.5 * (
    log(2 * pi)
    - log(precision_theta)
    + precision_theta
    * (theta - mu_hat_theta) ^ 2
  )
  return(log_prob)
}
```
...
The log conditional posterior distribution of $\sigma$ is implemented as follows:

```{r log-posterior-sigma}
log_posterior_sigma <- function(theta, sigma_sq, n, y_bar) {
  log_prob <- -0.5 * (
    log(sigma_sq) + sum(
      log(sigma_sq / n) + (y_bar - theta) ^ 2 / (sigma_sq / n)
    )
  )
  return(log_prob)
}

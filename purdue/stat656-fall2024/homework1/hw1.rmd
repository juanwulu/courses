...
2. (5 points) **Solution:**
    ...
    if (file.exists("data/covid_us.txt")) {
      ...
      stop("File not found: 'covid_us.txt' at ", getwd())
    }
    plot(
      x = seq_len(length(y)),
      y = y,
...
6. (10 points) **Solution:**
    ...
    ```{r fig.width=8, fig.height=6}
    y <- data[["cases"]]
    y <- diff(y[as.Date(data$date) >= as.Date("2020-06-30") &
                  as.Date(data$date) <= as.Date("2020-08-25")], lag = 1) / 100000
    samples <- matrix(NA, nrow = nrow(params), ncol = length(y))
    for (i in seq_len(nrow(params))) {
      samples[i, ] <- ar_post_predictive(params[i, "rho"], params[i, "log_sig"])
    }
    ...
    Based on the posterior predictive checks, the model provides a good prediction for the future data from June 30 to August 25. The skewness and kurtosis of the observed data are close to the expected values from the posterior predictive samples, and the time-series visualization shows that the posterior predictive samples are close to the observed data.
...
7. (10 points) **Solution:**
    ...
    # Read data
    if (file.exists("data/covid_us.txt")) {
      data <- read.table("data/covid_us-states.txt", header = TRUE, sep = ",")
      data <- data[data["state"] == "Indiana", ]  # Filter data for Indiana
      y <- data[["cases"]]
    } else {
      stop("File not found: 'covid_us-states.txt' at ", getwd())
    }

    y <- c(
      0,
      diff(y[as.Date(data$date) <= as.Date("2020-06-30")], lag = 1) / 1000
    )
    plot(
      x = seq_len(length(y)),
      y = y,
      type = "l",
      main = "Daily COVID-19 Case Increase in Indiana",
      xlab = "Date",
      ylab = "Cases",
    )
    ...
    y <- data[["cases"]]
    y <- diff(y[as.Date(data$date) >= as.Date("2020-06-30") &
                  as.Date(data$date) <= as.Date("2020-08-25")], lag = 1) / 1000
    samples <- matrix(NA, nrow = nrow(params), ncol = length(y))
    for (i in seq_len(nrow(params))) {
      samples[i, ] <- ar_post_predictive(params[i, "rho"], params[i, "log_sig"])
    }
    ...
    From the results, the model is capable of providing good predictions for the COVID-19 cases in Indiana. By comparing the time-series plot of cases in Indiana and in the US, the trend of the cases in Indiana is similar to the trend of the cases in the US. Therefore, the AR(1) model with the same prior can retain its predictive power transferring from the entire US to just Indiana.

8. (5 points) **Solution:**

    In summary, a AR(1) model on daily COVID-19 case increase is established in this problem. Specifically, the number of increased COVID-19 cases at the $t$-th day $\omega_t$ is linearly correlated with the number of increased cases at the $(t-1)$-th day $\omega_{t-1}$, with some random noise.
    $$
    \omega_t = \rho\omega_{t-1} + \epsilon_t, \quad \epsilon_t\sim\mathcal{N}(0, \sigma^2).
    $$

    The model is fitted to the data both in the entire US and in the Indiana state. We conclude that the model provides good predictions in both cases by drawing samples from its predictive distribution, and these samples have the distribution similar to the observed data. The success can be intuitively attributed to two main reasons: first, the similar trend of the COVID-19 cases in Indiana and the US, which makes the AR(1) model transferable from the entire US to Indiana; meanwhile, the differentiating data preprocess helps to remove the non-stationary trend in the data, which can not be captured by the AR(1) model.

\newpage

# Feedback

a. Yes, the instructor present the material in a clear and understandable way.
b. Yes, the instructor well-addressed the questions.
c. I would like to hear more about how to properly select the appropriate prior under different circumstances.
d. Yes, quite representative.
e. None that I can think of now.
f. I believe the instructor can provide more examples to illustrate posterior predictive checks for time-series data, which is more related to the homework.
g. No, I am satisfied with the course so far.
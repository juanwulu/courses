...
1. (25 points) **Solution**:
    ...
    Therefore, the weights $\beta_{i}$ can be both positive and negative. Given no prior knowledge on the features, I choose a non-informative prior for the weights, _i.e.,_ the isotropic standard normal distribution: $\mathcal{N}(0, \mathbf{I})$. The Stan model is implemented as follows:
    ...
    fig + scale_y_discrete(
      labels = parse(text = paste0("beta[", c(1:(ncol(x) - 1), 0), "]"))
    )
    ```
